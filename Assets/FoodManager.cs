...
void Start()
    {
        SM = GameObject.FindGameObjectsWithTag("SnakeManager").GetComponent<SnakeMovement>();
        SpawnFood();
    }
    void Update()
    {
    ...
    {
        float screenWidhtWorldPos = Camera.main.orthographicSize * Screen.width / Screen.height;
        float distBetweenBlocks = screenWidhtWorldPos / 5;
        for(int i=-2;i<3;i++)
        {
            float x = 2 * i * distBetweenBlocks;float y = 0;
